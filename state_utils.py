...
def batch_compute_invalid_moves(batch_state, batch_player, batch_ko_protect):
    ...
    # All pieces and empty spaces
    batch_all_pieces = np.sum(batch_state[:, [BLACK, WHITE]], axis=1)
...
def adj_data(state, action2d, player):
    ...
    valid_0 = (neighbors[:,0] < state.shape[1]).reshape((-1,1))
    valid_1 = (neighbors[:,1] < state.shape[2]).reshape((-1,1))
    valid = (neighbors >= 0) & np.concatenate((valid_0,valid_1),axis=1) # neighbors[:,0] < state.shape[0]
...
def batch_set_turn(batch_state):
    ...
    :return:
    """
    batch_state[:, TURN_CHNL] = 1 - batch_state[:, TURN_CHNL]
